...
def potential_evapotranspiration(monthly_temps_celsius,
    ...
    * *I* is a heat index which depends on the 12 monthly mean temperatures and is calculated as
    ...
    if len(data_shape) == 1:
        ...
        monthly_temps_celsius = monthly_temps_celsius.reshape((total_years, 12))

    elif (len(data_shape) > 2) or ((len(data_shape) == 2) and (data_shape[-1] != 12)):
        # logger.error(message)
        raise ValueError('Input monthly mean temperatures data array has an invalid shape.')
    ...
    for year in range(monthly_temps_celsius.shape[0]):
        actual_year = data_start_year + year
        ...
        pet[year, :] = 16 * (mean_daylight_hours / 12.0) * (month_days / 30.0) * (
                    (10.0 * monthly_temps_celsius[year, :] / I) ** a)
